...
public class SystemVectorMult2 extends SystemCode {
	
	public SystemVectorMult2() {
		name = "vectormult";
	...
	public Reference Invoke(Scope scope) {
		...
		if (len != v2.size()) {
			throw new ExceptionSemantic("The size of the vectors must be the same to do an add");
		}
		
		Vector<Reference> v3 = new Vector<Reference>(len);
		
		for (int i = 0; i < len; i++) {
			v3.add(new Reference(v1.get(i).getValue().mult(v2.get(i).getValue())));
		}
